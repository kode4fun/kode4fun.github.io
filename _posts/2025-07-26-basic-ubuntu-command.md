---
title: 基本 Ubuntu 命令
description: Ubuntu 系统的基本 shell 命令
author: kode4fun
...
| 命令  | 说明                                 | 示例                         |
| :---: | ------------------------------------ | ---------------------------- |
|  man  | 查看命令帮助，更复杂的还有 info      | `man cd`                     |
| help  | 查看 Linux 内置命令的帮助            | `help cd`                    |
|  cd   | 切换目录                             | `cd /` `cd ~` `cd -` `cd ..` |
|  pwd  | 显示当前工作目录的绝对路径           | `pwd -P` `pwd -L`            |
...
|   命令   | 说明                     | 选项                      | 示例                     |
| :------: | ------------------------ | ------------------------- | ------------------------ |
|    cp    | 复制文件或目录           | `-r` `-p` `-i`  `-u` `-v` | `cp -ri source target`   |
|    mv    | 移动或重命名文件         | `-i` `-u` `-v` `-f`       | `mv -iv old_loc new_loc` |
|    rm    | 删除一个或多个文件或目录 | `-i` `-f` `-r` `-v`       | `rm -iv /home/test`      |
|  mkdir   | 创建目录                 | `-m` `-p` `-v`            | `mkdir -m 777 -p a/b/c`  |
|  rmdir   | 删除空目录               |                           |                          |
| basename | 显示文件名或目录名       |                           |                          |
| dirname  | 显示文件或目录路径       |                           |                          |
