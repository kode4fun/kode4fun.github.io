...
XLSX.version = '0.8.11';
var current_codepage = 1200, current_cptable, cptable, style_builder;
...
function parse_ws_xml(data, opts, rels) {
	...
	// var columns = []; // del by fish
	var columns = [],rows = []; // added by fish
	...
	if(opts.cellStyles && data.indexOf("</rows>")!==-1) { // added by fish
		/* 18.3.1.13 col CT_Col */ // added by fish
		var _rows = data.match(/<row[^>]*\/>/g); // added by fish
		parse_ws_xml_rows(rows, _rows); // added by fish
	} // added by fish
	...
	if(rows.length > 0) s["!rows"] = rows; // added by fish
	console.log(`parse_ws_xml[xlsx-vite-style]:%o`,s["!rows"])
	return s;
}
...
function parse_ws_xml_rows(rows, _rows) { // added by fish
	var seencol = false; // added by fish
	for(var coli = 0; coli != _rows.length; ++coli) { // added by fish
		var coll = parsexmltag(_rows[coli], true); // added by fish
		var colm=parseInt(coll.min, 10)-1, colM=parseInt(coll.max,10)-1; // added by fish
		delete coll.min; delete coll.max; // added by fish
		if(!seencol && coll.width) { seencol = true; find_mdw(+coll.width, coll); } // added by fish
		if(coll.width) { // added by fish
			coll.wpx = width2px(+coll.width); // added by fish
			coll.wch = px2char(coll.wpx); // added by fish
			coll.MDW = MDW; // added by fish
		} // added by fish
		while(colm <= colM) rows[colm++] = coll; // added by fish
	} // added by fish
} // added by fish
...
function write_ws_xml_cols(ws, cols) {
	...
	o[o.length] = "</cols>";
	// console.log(`write_ws_xml_cols[xlsx-vite-style]: %o`, o);
	return o.join("");
}
// added by fish
function write_ws_xml_rows(ws, rows) {    // added by fish
	var o = ["<rows>"], row, height;    // added by fish
	for(var i = 0; i != rows.length; ++i) {    // added by fish
		if(!(row = rows[i])) continue;    // added by fish
		// var p = {min:i+1,max:i+1};    // added by fish
		var p = {r:i+1};    // added by fish
		/* hpt (chars), hpx (pixels) */    // added by fish
		height = -1;    // added by fish
		if(row.hpx) height = px2char(row.hpx);    // added by fish
		else if(row.hpt) height = row.hpt;    // added by fish
		if(height > -1) { 
		p.ht= Math.round(char2width(height)); 
		// p.height = p.ht;
		p.customHeight= 1;
		// p["x14ac:dyDescent"] ="0.25";
		}    // added by fish
		// console.log(`write_ws_xml_rows[xlsx-vite-style]A: %o`, p);
		o[o.length] = (writextag('row', null, p));    // added by fish
	}    // added by fish
	o[o.length] = "</rows>";    // added by fish
	// console.log(`write_ws_xml_rows[xlsx-vite-style]B: %o`, o);
	return o.join("");    // added by fish
}    // added by fish


function write_ws_xml_cell(cell, ref, ws, opts, idx, wb) {
	// if(cell.v === undefined && cell.s === undefined) return ""; // by fish
	// console.log(`write_ws_xml_cell[xlsx-vite-style] cell ==> %o\nref ==> %o\nws ==> %o\nopts ==> %o\nidx ==> %o\nwb ==> %o`,cell,ref,ws,opts,idx,wb);
	if (cell.v === undefined && cell.s === undefined && !cell.f) return ''; // by fish
	var vv = "";
	...
	// var v = writetag('v', escapexml(vv)), o = {r:ref};// by fish
	var o = { r: ref },v = cell.f ?writetag('f', escapexml(cell.f)) :writetag('v', escapexml(vv)); // by fish
	/* TODO: cell style */
...
function write_ws_xml_data(ws, opts, idx, wb) {
	// console.log(`write_ws_xml_data[xlsx-vite-style:7868]ws ==> %o`,ws);
	...
	for(R = range.s.r; R <= range.e.r; ++R) {
		...
		// if(r.length > 0) o[o.length] = (writextag('row', r.join(""), {r:rr})); // del by fish
		// 将数据写到 <c ....></c> 里面，高度值来自 !row 的 hpx，注意需要 customHeight 域
		const rowTag = {r:rr}; // added by fish
		if(ws['!rows'] && ws['!rows'][rr-1] && Number.parseFloat(ws['!rows'][rr-1].hpx)) // added by fish
			Object.assign(rowTag,{ht:Number.parseFloat(ws['!rows'][rr-1].hpx),customHeight:'1'}); // added by fish
		if(r.length > 0) o[o.length] = (writextag('row', r.join(""), rowTag)); //added by fish
		// console.log(`write_ws_xml_data[xlsx-vite-style:7880]r ==> %o\nrr ==> %o`,r,rr);
	}
	// console.log(`write_ws_xml_data[xlsx-vite-style:7882]o ==> %o`,o);
	return o.join("");
}
...
var WS_XML_ROOT = writextag('worksheet', null, {
	'xmlns': XMLNS.main[0],
	'xmlns:r': XMLNS.r,
	'xmlns:mc':'http://schemas.openxmlformats.org/markup-compatibility/2006', // added by fish
	'mc:Ignorable':'x14ac' , // added by fish
	'xmlns:x14ac':'http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac', // added by fish
});
...
function write_ws_xml(idx, opts, wb) {
  ...
  o[o.length] = writextag('sheetViews', sheetView);
	...
	if(ws['!rows'] !== undefined && ws['!rows'].length > 0) o[o.length] = (write_ws_xml_rows(ws, ws['!rows']));// added by fish
...
var XLS = XLSX;
console.log('XLSX-style-vite loaded')